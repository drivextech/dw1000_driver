...
const static nrf_drv_spi_t g_spi_inst = NRF_DRV_SPI_INSTANCE(SPI_INSTANCE);  /**< SPI instance. */
volatile static bool g_spi_xfer_done;  /**< Flag used to indicate that SPI instance completed the transfer. */
 ...
 *
 *******************************************************************************/
volatile static locker_func_t g_locker = NULL;
volatile static unlocker_func_t g_unlocker = NULL;
...
/* @fn      port_set_mutexer
 * @brief   set mutex locker/unlocker function to be used when do SPI transmission
 * */
...
int readfromspi(uint16 headerLength, const uint8 *headerBuffer, uint32 readlength, uint8 *readBuffer)
{
    ...
    if(g_locker) g_locker();
    g_spi_xfer_done = false;
    nrf_drv_spi_transfer(&g_spi_inst, idatabuf, idatalength, itempbuf, idatalength);
    while(!g_spi_xfer_done) ;
    if(g_unlocker) g_unlocker();
...
void port_set_dw1000_slowrate(void)
{
	...
	APP_ERROR_CHECK( nrf_drv_spi_init(&g_spi_inst, &spi_config, spi_event_handler, NULL) );
	nrf_delay_ms(2);
}
